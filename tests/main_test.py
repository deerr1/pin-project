...
class MainTestCase(unittest.TestCase):
    ...
    def test_incorrect_input(self):
        ...
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > label:nth-child(2) > div > div.q-field__bottom.row.items-start.q-field__bottom--animated > div > div')
        self.assertEqual(elem.text, 'Введите логин', 'Нет сообщения об ошибке')

        # Переход на страницу регистрации
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > div:nth-child(5) > button > span.q-btn__content.text-center.col.items-center.q-anchor--skip.justify-center.row > span')
        elem.click()
        time.sleep(1)

        # Некорректный ввод почты
        elem: WebElement = self.driver.find_element(By.XPATH, "//form[@class='q-gutter-md']/label[2]//input")
        elem.send_keys('jojir90142')
        time.sleep(1)

        # Отправить форму регистрации
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > div:nth-child(6) > button')
        elem.send_keys(Keys.ENTER)
        time.sleep(1)

        # Проверка предупреждений
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > label:nth-child(3) > div > div.q-field__bottom.row.items-start.q-field__bottom--animated > div > div')
        self.assertEqual(elem.text, 'Вы неправильно ввели почту', 'Нет сообщения об ошибке')

        # Ввод пароля
        elem: WebElement = self.driver.find_element(By.XPATH, "//form[@class='q-gutter-md']/label[3]//input")
        elem.send_keys('qwe12')
        time.sleep(1)

        # Неверный повтор пароля
        elem: WebElement = self.driver.find_element(By.XPATH, "//form[@class='q-gutter-md']/label[4]//input")
        ...
        # Проверка предупреждений
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > label:nth-child(4) > div > div.q-field__bottom.row.items-start.q-field__bottom--animated > div > div')
        self.assertEqual(elem.text, 'Пароль слишком короткий', 'Нет сообщения об ошибке')

        # Проверка предупреждений
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > label:nth-child(5) > div > div.q-field__bottom.row.items-start.q-field__bottom--animated > div > div')
        self.assertEqual(elem.text, 'Введенный пароль не равен введеному выше', 'Нет сообщения об ошибке')
    
    def test_registration(self):
        
        self.driver.get("http://localhost:8082/#/Registration")
        self.assertEqual(self.driver.title, 'pin', 'Открыто не то окно')
        
        # Заполнение формы регистрации
        elem: WebElement = self.driver.find_element(By.XPATH, "//form[@class='q-gutter-md']/label[1]//input")
        ...
        for _ in range(20):
            time.sleep(1)
            try:
                elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > h4')
    ...
    def test_check_registered_user(self):
        ...
        # Проверка предупреждений
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#window_reg > form > div.q-mx-xl.text-red')
    ...
    def test_add_boards(self):
        ...
        # Перейти в профиль
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(6) > div > div > div:nth-child(1) > div.q-item__section.column.q-item__section--main.justify-center > div')
        elem.click()
        time.sleep(1)

        # Добавить открытую доску
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#app > div:nth-child(2) > div.q-pa-md > div > div > div.q-tab-panels.q-panel-parent > div > div > div.Button > button')
        elem.click()
        time.sleep(0.2)

        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(6) > div > div > div:nth-child(2)')
        elem.click()
        time.sleep(0.2)

        # Проверка отправки пустых полей
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(7) > div > div.q-dialog__inner.flex.no-pointer-events.q-dialog__inner--minimized.q-dialog__inner--standard.fixed-full.flex-center > div > div > form > div:nth-child(4) > button.q-btn.q-btn-item.non-selectable.no-outline.q-btn--standard.q-btn--rectangle.bg-orange.text-white.q-btn--actionable.q-focusable.q-hoverable')
        elem.click()
        time.sleep(0.2)

        # Проверка предупреждений
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(7) > div > div.q-dialog__inner.flex.no-pointer-events.q-dialog__inner--minimized.q-dialog__inner--standard.fixed-full.flex-center > div > div > form > label.q-field.row.no-wrap.items-start.q-field--outlined.q-input.q-field--rounded.q-field--labeled.q-field--error.q-field--highlighted.q-field--with-bottom > div > div.q-field__bottom.row.items-start.q-field__bottom--animated > div > div')
        ...
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#app > div:nth-child(2) > div.q-pa-md > div > div > div.q-tab-panels.q-panel-parent > div > div > div.folders > div:nth-child(1) > div > div.q-img__content.absolute-full.q-anchor--skip > div')
        self.assertEqual(elem.text.split('\n')[0], "Jojir's board", 'Название доски не соответствует заданному')
        ...
        elem: WebElement = self.driver.find_element(By.XPATH, "//div[@class='q-virtual-scroll__content']/div[2]")
        elem.click()
        time.sleep(0.2)
        ...
        # Проверка создания доски
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, '#app > div:nth-child(2) > div.q-pa-md > div > div > div.q-tab-panels.q-panel-parent > div > div > div.folders > div:nth-child(2) > div > div.q-img__content.absolute-full.q-anchor--skip > div')
        self.assertEqual(elem.text.split('\n')[0], "Jojir's board lock", 'Название доски не соответствует заданному')
    ...
    def test_add_pin(self):
        ...
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(6) > div > div > div:nth-child(1)')
        elem.click()
        time.sleep(0.5)

        # Отправить пустую форму
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(7) > div > div.q-dialog__inner.flex.no-pointer-events.q-dialog__inner--minimized.q-dialog__inner--standard.fixed-full.flex-center > div > form > div.row.self-center.q-mx-lg.q-mt-xl > button.q-btn.q-btn-item.non-selectable.no-outline.q-btn--standard.q-btn--rectangle.bg-orange.text-white.q-btn--actionable.q-focusable.q-hoverable')
        elem.click()
        time.sleep(0.5)
        ...
        # Описание пина
        elem: WebElement = self.driver.find_element(By.XPATH, "//form[@class='q-form column justify-around q-mt-lg']//label[2]//textarea")
        elem.send_keys("Красивый попуг")
        time.sleep(0.2)

        # Загрузка изображения
        img = str(Path(__file__).parent.absolute()) + '/img/popug.jpg'
        elem: WebElement = self.driver.find_element(By.CSS_SELECTOR, 'body > div:nth-child(7) > div > div.q-dialog__inner.flex.no-pointer-events.q-dialog__inner--minimized.q-dialog__inner--standard.fixed-full.flex-center > div > form > div.row.justify-between.q-mx-lg > div.col-5.column.justify-between > div > div.q-uploader__header.bg-orange > div > a > span.q-btn__content.text-center.col.items-center.q-anchor--skip.justify-center.row > input')
        elem.send_keys(img)
        time.sleep(2)
    ...
    def test_save_other_user_pin(self):
        ...
        # Создать доску
        elem: WebElement = self.driver.find_element(By.XPATH, "//div[@class='q-menu q-position-engine scroll']/button")
        elem.click()
        time.sleep(0.5)
        ...
        self.driver.get('http://localhost:8082/#/profile/popug')
        time.sleep(2)
